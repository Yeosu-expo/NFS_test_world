...
def main():
    ...
    def run_case(slow_node_count: int, staleness_k: int) -> None:
        ...
        steps_axis: List[int] = []
        cos_fast_slow: List[float] = []  # fast(1) vs STALE slow(1)
        ...
        stale_slow_avg_vec = None  # holds a single slow rank's grad (frozen)
            ...
            if world_size > 1 and len(fast_ranks) > 0 and len(slow_ranks) > 0 and fast_pg is not None and slow_pg is not None:
                ...
                if step_idx == last_slow_grad_step:
                    if is_slow_rank and grad_vec is not None:
                        # choose one slow node (slow_root) gradient and broadcast
                        if rank == slow_root:
                            slow_one = grad_vec.clone()
                        else:
                            slow_one = torch.empty_like(grad_vec)
                        # share slow_one and intra_slow scalar to world for caching on rank 0
                        dist.broadcast(slow_one, src=slow_root, group=dist.group.WORLD)
                        ...
                        slow_one = torch.empty_like(_flatten_grads(model, device=device))
                        dist.broadcast(slow_one, src=slow_root, group=dist.group.WORLD)
                        intra_slow_tensor = torch.empty(1, device=device, dtype=torch.float32)
                        dist.broadcast(intra_slow_tensor, src=slow_root, group=dist.group.WORLD)

                    if rank == 0:
                        stale_slow_avg_vec = slow_one.detach().clone()
                        slow_once_similarity = float(intra_slow_tensor.item())

                # Cross-group cosine each step vs STALE slow(1) grad
                cos_fs = float('nan')
                if is_fast_rank and grad_vec is not None:
                    # Gather fast grads and select a single fast rank's vector (min fast rank)
                    gathered_fast = _gather_group_vectors(grad_vec, group=fast_pg, group_size=len(fast_ranks))
                    if rank == 0 and stale_slow_avg_vec is not None and len(gathered_fast) == len(fast_ranks):
                        fast_sel_rank = min(fast_ranks)
                        fast_sel_idx = fast_ranks.index(fast_sel_rank)
                        fast_one = gathered_fast[fast_sel_idx]
                        if fast_one.numel() == stale_slow_avg_vec.numel():
                            cos_fs = F.cosine_similarity(fast_one, stale_slow_avg_vec, dim=0, eps=1e-8).item()
        ...
        if rank == 0:
            ...
            # 1) fast(1) vs slow(1) cosine
            if steps_axis:
                plt.figure()
                plt.plot(steps_axis, cos_fast_slow, marker='o', linestyle='-')
                plt.xlabel('step')
                plt.ylabel('cosine similarity')
                plt.title(f'Fast(1)-vs-Slow(1) grad — sn={slow_node}, K={k_sync}')
                plt.ylim([-1.0, 1.0])
